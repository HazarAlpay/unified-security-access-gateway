...
1. Backend SetupBash# Navigate to the backend directory
cd backend
...
⚠️ Configuration (.env)Create a file named .env inside the backend folder and add the following variables:Kod snippet'iSECRET_KEY=your_super_secret_key_here
ALGORITHM=HS256
ACCESS_TOKEN_EXPIRE_MINUTES=30
GOOGLE_API_KEY=your_google_gemini_api_key
Note: GOOGLE_API_KEY is required for the AI Rule Engine to work. You can get one from Google AI Studio.Database InitializationInitialize the database and create default users:Bashpython seed.py

# Output: "Database seeded successfully!"
Start the ServerBashuvicorn main:app --reload

# Server will start at http://127.0.0.1:8000

2. Frontend SetupOpen a new terminal and navigate to the frontend directory:Bash# Navigate to the frontend directory
cd frontend
...
3. Access the ApplicationOpen your browser and navigate to:http://localhost:5173🔐 Default CredentialsUse the following credentials to log in:RoleUsernamePasswordAdminadminadmin123Employeeemployeeuser123
